...
const TOKEN_STORAGE_KEY = "agentic-ai-token";
const EMAIL_STORAGE_KEY = "agentic-ai-email";
...
export default function Home() {
  ...
  async function submitFeedback(rating: "up" | "down") {
    if (!token || !lastResponse || !lastQuestion) {
      return;
    }
    ...
    try {
      ...
      const data = (await response.json()) as { review_required: boolean };
      setFeedbackStatus(
        data.review_required
          ? "Review queued for this answer."
          : "Feedback recorded for evaluation."
      );
      appendLog("INFO", `Answer feedback recorded: ${rating}.`);
    } catch (error) {
          ...
          <div style={authFrameStyle}>
            ...
            <div style={authGridStyle}>
              <div style={styles.authIntro}>
                <div style={styles.sectionKicker}>agentic ai workspace</div>
            ...
            <div style={{ ...terminalCardStyle, ...styles.logsPanelBox }}>
              ...
              <div style={styles.terminalBody}>
                ...
                {lastResponse ? (
                  <div style={styles.terminalBlock}>
                    <div style={styles.terminalBlockTitle}>
                      <ShieldCheck size={10} style={{ display: "inline-block", verticalAlign: "middle", marginRight: 4 }} />
                      evaluation
                    </div>
                    {lastResponse.review_required ? (
                      <div style={styles.reviewNotice}>
                        {lastResponse.review_reason ?? "Human review recommended."}
                      </div>
                    ) : null}
                    <div style={styles.feedbackActions}>
                      ...
                      <button
                        ...
                        style={styles.feedbackButton}
                        aria-label="Request answer review"
                      >
                        <ThumbsDown size={13} /> review
                      </button>
                    </div>
                    <div style={styles.feedbackStatus}>{feedbackStatus || "Awaiting evaluator input."}</div>
                  </div>
                ) : null}
