...
start "Agentic AI - Backend" powershell -NoExit -ExecutionPolicy Bypass -Command "if (Get-NetTCPConnection -LocalPort 8000 -State Listen -ErrorAction SilentlyContinue) { Write-Host 'Backend already running on port 8000.'; exit 0 }; Set-Location -LiteralPath '%ROOT%backend'; $env:REDIS_URL='redis://127.0.0.1:1/0'; $env:ELASTICSEARCH_URL='http://127.0.0.1:1'; & '..\.venv\Scripts\python.exe' -m uvicorn app.main:app --host 127.0.0.1 --port 8000"
