...
from app.workflows.routing_graph import RoutingGraph
...
class ChatWorkflow:
    def __init__(self) -> None:
        ...
        self.routing_graph = RoutingGraph(self.supervisor)
    ...
    async def run(self, request: ChatRequest) -> ChatResponse:
        ...
        state.route = await self.routing_graph.decide(
            request.message,
            feedback_hint=self.feedback_service.routing_signal(),
        )
        ...
        response = ChatResponse(
            ...
            review_required=not bool(state.search_results) and state.route in {"search", "parallel"},
            review_reason=(
                "No indexed evidence was available for this answer. Review or ingest more documents."
                if not state.search_results and state.route in {"search", "parallel"}
                else None
            ),
        )
        clear_log_context()
